/* ============================================================
   CIVIL AT HAND — Main Script v3.0
   
   HOW TO EDIT:
   - Phone number: search 8708524647 and replace globally
   - To disable hero canvas: set HERO_CANVAS = false
...
/* ===================================================
   PAGE LOADER — 900ms (down from 1700ms)
   =================================================== */
...
// Observe all reveal elements not in hero (hero is handled by loader)
...
document.querySelectorAll('.why-card-main').forEach(el => revealObserver.observe(el));
...
(function initHeroCanvas() {
  ...
  function draw() {
    ...
    buildings.forEach(b => {
      ...
      for (let c = 0; c < cols; c++) {
        for (let r = 0; r < b.floors - 1; r++) {
          ...
          glow.addColorStop(0, GOLD + '0.28)');
          glow.addColorStop(1, GOLD + '0.04)');
          ctx.fillStyle = glow;
    ...
    const hg = ctx.createLinearGradient(0, ground - 60, 0, ground + 20);
    hg.addColorStop(0, GOLD + '0.05)');
  ...
  window.addEventListener('resize', () => {
    ...
    window._rt = setTimeout(resize, 220);
  }, { passive: true });
  draw();
...
/* ===================================================
   EXPAND / READ MORE (Student guidance page)
   =================================================== */
...
/* ===================================================
   ...
   Replace with Formspree / EmailJS / Netlify Forms
   in production.
   =================================================== */
...
/* ===================================================
   PROGRESS BAR — CSS var width trick
   =================================================== */
document.querySelectorAll('.wcp-fill').forEach(fill => {
  const w = fill.getAttribute('data-w') || fill.style.width;
  fill.style.setProperty('--w', w);
  fill.style.width = '0';
});

// Animate bars when their card scrolls into view
...
document.querySelectorAll('.why-card-main').forEach(el => barObserver.observe(el));

/* ===================================================
   CALCULATORS
   (tools.html — all live-update calculator functions)
   =================================================== */
...
function calcTile() {
  ...
  const perBox      = tileSize === 600 ? 4 : tileSize === 800 ? 2 : tileSize === 300 ? 9 : 4;
...
function calcSteel() {
  ...
  // IS standard formula: D²/162 kg/m
...
/* ===================================================
   INJECT KEYFRAMES (spin for form button)
   =================================================== */
